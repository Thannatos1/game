...
(function(){
  ...
  function currentUserRank(){ return n(compSummary?.user_rank); }
  function currentUserScore(){ return n(compSummary?.user_score || best || 0); }
  ...
  function getDisplayRows(){
    ...
    if (rank === 1) {
      return uniqueRows(
        compTop.filter(r => n(r.rank) > 1).slice(0, 3)
      );
    }

    if (rank <= 3) {
      return uniqueRows(
        (compAround || []).filter(r => n(r.rank) !== rank).sort((a,b)=>n(a.rank)-n(b.rank))
      );
    }

    return uniqueRows(compAround || []);
  }
  ...
  function getObjectiveState(){
    ...
    const top5Score = n(compSummary?.top_cutoff_score);
    const rank2 = rowByRank(compTop, 2);
    const rank3 = rowByRank(compTop, 3);
    const below = rowByRank(getDisplayRows().concat(compTop), rank + 1);
    const nextPts = n(compSummary?.points_to_next);
    const top5Pts = n(compSummary?.points_to_top);
    const nextName = String(compSummary?.next_better_name || 'próximo rival').toUpperCase();
  ...
  async function loadRankingsCompetitivePhase2(){
    if (!networkOnline) {
    ...
    try {
      ...
      compTop = Array.isArray(data?.top) ? data.top : [];
      compAround = Array.isArray(data?.around_me) ? data.around_me : [];
      compSummary = data?.summary || null;

      rankings = compTop.slice();
      userPosition = n(compSummary?.user_rank) > 0 ? n(compSummary.user_rank) - 1 : -1;

      if (typeof trackEvent === 'function') {
        trackEvent('ranking_loaded', {
          total_players: n(compSummary?.total_players),
          user_rank: n(compSummary?.user_rank),
          source: 'competitive_phase2'
        });
      }
      return true;
    } catch (e) {
      rankingsError = 'Erro ao carregar';
      console.error('[Orbita] loadRankingsCompetitivePhase2 failed', e);
      if (typeof trackEvent === 'function') {
  ...
  function drawCompetitiveRow(r, x, y, w, h, mode){
    ...
    if (isUser) {
      ...
      X.fillText(mode === 'top' ? 'TOP GLOBAL' : getDisplayTitle(), x + 82, y + h/2 + 9);
    }
  ...
  function drawRankingMenuCompetitivePhase2(){
    drawTopStatusBadges();
    X.textAlign='center'; X.textBaseline='middle';
    ...
    X.fillText(fmtRank(compSummary?.user_rank), sx+14, y+44);
    ...
    X.fillText(String(compSummary?.total_players || 0), W/2, y+39);
    ...
    X.fillStyle='#fff';
    X.font='bold 13px -apple-system, system-ui, sans-serif';
    X.fillText((compSummary?.percentile != null ? compSummary.percentile + '%' : '--'), W/2, y+76);

    X.textAlign='right';
    X.fillStyle='rgba(255,255,255,0.52)';
    X.font='10px -apple-system, system-ui, sans-serif';
    X.fillText(state.right1Label, sx+sw-14, y+16);
    X.fillStyle=state.right1Color;
    X.font='bold 18px -apple-system, system-ui, sans-serif';
    X.fillText(state.right1Value, sx+sw-14, y+39);

    X.fillStyle='rgba(255,255,255,0.52)';
    X.font='10px -apple-system, system-ui, sans-serif';
    X.fillText(state.right2Label, sx+sw-14, y+62);
    X.fillStyle=state.right2Color;
    X.font='bold 13px -apple-system, system-ui, sans-serif';
    X.fillText(state.right2Value, sx+sw-14, y+76);
    ...
    for (const r of (compTop || []).slice(0, 3)) {
    ...
    X.fillStyle='#fff';
    X.font='bold 13px -apple-system, system-ui, sans-serif';
    X.fillText(state.message, sx+12, y+35);

    X.fillStyle='rgba(255,255,255,0.42)';
    X.font='10px -apple-system, system-ui, sans-serif';
    X.fillText(state.sub, sx+12, y+49);
  }

  loadRankings = loadRankingsCompetitivePhase2;
  drawRankingMenu = drawRankingMenuCompetitivePhase2;
})();
